...
本项目脱胎于一个计算传播学课程作业（"鹅腿阿姨"事件舆情分析）。原脚本把**事件日期、核心短语词典、绝对路径、Cookie** 全部硬编码，换个事件就得改一堆代码。本仓库把它们全部抽成**配置 + 自动推断**，让任何人只需做两件事即可在本地复现整套分析：

1. 填入想分析的舆情主题（例如「鹅腿阿姨」）
2. 填入自己的 B 站 Cookie
...
编辑 `config.yaml`，至少填好 `project.topic`（如 `鹅腿阿姨`）。
